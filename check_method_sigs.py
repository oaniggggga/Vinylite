...
for m in range(method_count):
    ...
    method_name = cp[name_idx][1] if name_idx < len(cp) else '???'
    method_desc = cp[desc_idx][1] if desc_idx < len(cp) else '???'
    attr_count = struct.unpack('>H', f.read(2))[0]
    has_sig = False
    for _ in range(attr_count):
        ...
        attr_name = cp[name_idx2][1] if name_idx2 < len(cp) else '???'
    ...
    if not has_sig and ('<' in method_desc or 'T' in method_desc):
        print(f'  {method_name} {method_desc} -> NO SIGNATURE')